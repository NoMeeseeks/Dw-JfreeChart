...
import org.example.conexcion.conexcionDBPostGres;
...
public class usuariosDAO {
    ...
    public ArrayList<usuariosXsueldo> obtenerUsuariosPorSueldo(){
        ...
        //se realiza la conexcion a la base de datos y si es verdadero se ejecuta el try
        try(
                Connection con = conexcionDBPostGres.obtenerConexcionPostgres();
                Statement statement = con.createStatement();
                ResultSet rs = statement.executeQuery("select count(u.idusuario)as cantidad,u.sueldo  from usuarios u group by u.sueldo ;"))
        {
            while (rs.next()){
